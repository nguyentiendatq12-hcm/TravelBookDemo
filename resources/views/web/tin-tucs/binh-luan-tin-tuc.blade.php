...
<ul class="comment-list">
    @foreach ($lsbinhluancha as $key => $value)
        ...
        <div class="post-comt-box rep-commment" id="rep-commment{{ $value->id }}">
            <div class="p-5" id="rep-binh-luan-bai-viet">
                @csrf
                ...
                <div class="form-group">
                    <button  onclick="rep_binh_luan_tin_tuc({{ $value->bai_viet_id }},{{ $value->id }}, {{ $trang }})" class="btn py-3 px-4 btn-primary">{{ trans('public.comment') }}</button>
                </div>
